...
<body>
	...
	<%
	UserDbOperation sb = new UserDbOperation();
	List<User> user = sb.getAll();
	%>


	<section class="container-flex mt-2  mx-auto">
		<h1 class="text-center mb-3">User's Information</h1>
			...
			<table id="cart"
				...
				<tbody>
					...
					<tr>
						...
						<td data-th="Price"><%=u.getEmail() %></td>
						<td data-th="Price"><%=u.getMno() %></td>
						<td data-th="Price"><%=u.getAddress() %></td>
						<td data-th="Price"><%=u.getPass() %></td>

						<td class="actions " data-th=""><a
							class="btn btn-danger btn-sm mt-2" href="AdminUser?uid=<%=u.getId()%>">
								<i class="fa-solid fa-arrows-rotate"> Update</i>
						</a> <a class="btn btn-danger btn-sm mt-2 " href="AdminUser?did=<%=u.getId()%>" >
